...
def _make_item(directory, vcsp_type, name, files, description="", properties={},
    ...
    if vcsp_type == VCSP_TYPE_OVF:
        ...
        type_metadata_value = "{\"id\":\"%s\",\"version\":\"%s\",\"libraryIdParent\":\"%s\",\"isVappTemplate\":\"%s\",\"vmTemplate\":null,\"vappTemplate\":null,\"networks\":[],\"storagePolicyGroups\":null}" % (
...
def _dir2item_s3(s3_client, bucket_name, path, item_name, skip_cert, lib_id, old_item=""):
        ...
        if ".ovf" in file_name:
            ...
            if "type-metadata" not in old_item:
                try:
                    ...
                    logging.error("Failed to read ovf descriptor: %s" % file_path)
    ...
    for content in response['Contents']:
        ...
        last_modified = content[
            'LastModified']  # sample 'LastModified': datetime.datetime(2018, 7, 23, 16, 24, 3, tzinfo=tzutc())
        file_json = {
            "name": file_name,
            "size": size,
        ...
        if vcsp_type == VCSP_TYPE_ISO:
            ...
            items_json[child_item_name] = _make_item(item_path, vcsp_type, child_item_name, [file_json],
    ...
    if vcsp_type != VCSP_TYPE_ISO:
        ...
        items_json[item_name] = _make_item(item_name, vcsp_type, item_name, files_items, identifier=identifier,
                                           library_id=lib_id, is_vapp_template=is_vapp)
...
def make_vcsp_s3(conn_vars, skip_cert):
    """
    ...
    is deleted later.
    """
    region_name = conn_vars['region_name']
    endpoint_url = conn_vars['endpoint_url']
    aws_access_key_id = conn_vars['aws_access_key_id']
    aws_secret_access_key = conn_vars['aws_secret_access_key']
    lib_name = conn_vars['library_name']
    lib_path = conn_vars['library_path']
    ...
    try:
        ...
        # raise Exception("The given bucket %s doesn't exist or you have no access to it" % bucket_name)
        logging.error(f"Bucket check error: {e}", exc_info=False)
        return f'Failed ( {str(e)})'

    # Path: lib.json and items.json (root of Library folder)
    ...
    if file_exist_on_s3(s3_client, bucket_name, lib_json_path):
        logging.info("%s already exists (%s)" % (LIB_FILE, lib_json_path))
        try:
            ...
            logging.error("Failed to read %s" % lib_json_path)
            pass

    old_items = {}
    # if items.json exist
    if file_exist_on_s3(s3_client, bucket_name, lib_items_json_path):
        logging.info("%s already exists (%s)" % (ITEMS_FILE, lib_items_json_path))
        try:
            ...
            logging.error("Failed to read %s" % lib_items_json_path)
            pass
    ...
    if updating_lib and not changed:
        logging.info("Nothing to update on the library")
        if update_items_json:
            logging.info("items json needs to be updated, updating items json...")
    ...
    logging.info("Saving results to %s and %s" % (lib_json_path, lib_items_json_path))
...
def reindex_bucket(conn_vars):
    """
    MinIO S3 Endpoint URL: From env. Variable
    Access Key Id: From env. Variable
    Secret Access Key: From env. Variable
    Library Name: From Webook POST
    Library Path: From Webook POST
    :return: Status with Message
    """
    # logging.info(conn_vars)
    storage_type = "s3"
    md5_enabled = "True"
    skip_certificate = "True"

    status = make_vcsp_s3(conn_vars, skip_certificate)

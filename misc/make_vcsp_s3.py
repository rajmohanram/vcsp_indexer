...
def _dir2item(path, directory, md5_enabled, lib_id):
    ...
    return _make_item(name, vcsp_type, name, files_items, identifier=uuid.uuid4(), library_id=lib_id,
...
def make_vcsp_s3(conn_vars, skip_cert):
    ...
    logging.info(f"Library Path: {lib_path}")
    logging.info(f"Bucket Name: {bucket_name}")
    logging.info(f"Library Folder Path: {lib_folder_path}")
...
if __name__ == "__main__":
    ...
    connection_vars = {
        ...
        'library_name': os.environ.get('lib_name'),
        'library_path': os.environ.get('lib_path')
    }
    ...
    if connection_vars_none:
        logging.info("Following connection variables are set to None")
        for name in connection_vars_none:
            logging.info(f"'{name}' is not set.")
        sys.exit()
    ...
    logging.info(f"Staring to index S3 bucket contents.")
    make_vcsp_s3(connection_vars, skip_certificate)
